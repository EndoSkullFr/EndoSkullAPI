...
public class RankInventory extends CustomGui {
    public RankInventory(Player p) {
        ...
        setItem(24, new CustomItemStack(Material.NAME_TAG).setName(Languages.getLang(p).getMessage(MessageUtils.Global.SHOP_HERO)).setLore(Languages.getLang(p).getMessage(MessageUtils.Global.SHOP_HERO_DESC)), player -> BoutiqueInventory.sendStoreLink(player));
